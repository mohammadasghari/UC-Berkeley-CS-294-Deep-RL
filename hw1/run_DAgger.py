...
Example usage:
    ...
    python3.5 run_DAgger.py expert_data/Humanoid-v2.pkl Humanoid-v2 --num_rollouts 10
    python3.5 run_DAgger.py expert_data/Ant-v2.pkl Ant-v2 --num_rollouts 10
    python3.5 run_DAgger.py expert_data/Hopper-v2.pkl Hopper-v2 --num_rollouts 10
    python3.5 run_DAgger.py expert_data/Reacher-v2.pkl Reacher-v2 --num_rollouts 10
    python3.5 run_DAgger.py expert_data/Walker2d-v2.pkl Walker2d-v2 --num_rollouts 10
...
def main():
    ...
    observations_data = np.array(expert_data['observations'])
    actions_data = expert_data['actions']
    actions_data = np.squeeze(actions_data, axis=1)
    print('loaded and two numpy arrays of observations and action built')

    data_size = np.shape(observations_data)[0]
    obs_size = np.shape(observations_data)[1]
    actions_size = np.shape(actions_data)[1]
    ...
    for training_step in range(num_steps):
        ...
        input_data = observations_data[indices]
        output_data = actions_data[indices]
    ...
    for i in range(args.num_rollouts):
        ...
        # add the new observations and actions to the data for supervised learning

        observations_data = np.concatenate((observations_data, observations_array), axis=0)
        actions_data = np.concatenate((actions_data, actions_array), axis=0)

        # perform supervised learning on the updated dataset
    ...
    with open(os.path.join('returns_data', args.envname + 'DAgger' + '.pkl'), 'wb') as f:
