...
class ModelBasedPolicy(object):
    ...
    def _dynamics_func(self, state, action, reuse):
        ...
        normalized_state_diff = utils.build_mlp(input_layer=normalized_state_action,
        ...
        unnormalized_state_diff = utils.unnormalize(normalized_state_diff, self._init_dataset.delta_state_mean,
                                                    self._init_dataset.delta_state_std)

        next_state_pred = state + unnormalized_state_diff
    ...
    def _setup_training(self, state_ph, next_state_ph, next_state_pred):
        ...
        normalized_actual_state_diff = utils.normalize(actual_state_diff, self._init_dataset.delta_state_mean,
                                                       self._init_dataset.delta_state_std)

        normalized_pred_state_diff = utils.normalize(pred_state_diff, self._init_dataset.delta_state_mean,
                                                     self._init_dataset.delta_state_std)

        loss = tf.losses.mean_squared_error(labels=normalized_actual_state_diff, predictions=normalized_pred_state_diff)
    ...
    def _setup_action_selection(self, state_ph):
        ...
        total_cost = np.zeros(self._num_random_action_selection)

        for i in range(self._horizon):

            # action_ph = tf.random_uniform(
            #     shape=[self._num_random_action_selection, self._action_dim],
            #     minval=self._action_space_low, maxval=self._action_space_high,
            #     dtype=tf.float32)
            #
            # if i == 0:
            #     initial_actions = action_ph

            action_ph = action_sequences[i, :, :]

            next_state_pred_ph = self._dynamics_func(state_ph, action_ph, reuse=True)
            current_costs = self._cost_fn(state_ph, action_ph, next_state_pred_ph)

            total_cost = total_cost + current_costs

            state_ph = next_state_pred_ph

        best_action_index = tf.argmin(total_cost, axis=0)

        best_action = action_sequences[0, best_action_index, :]
    ...
    def _setup_graph(self):
        ...
        # print('Placeholder for next state prediction created!')
    ...
    def train_step(self, states, actions, next_states):
        ...
        _, loss = self._sess.run([self._optimizer, self._loss], feed_dict=feed_dict)
    ...
    def predict(self, state, action):
        ...
        feed_dict = {self._state_ph: [state], self._action_ph: [action]}
    ...
    def get_action(self, state):
        ...
        # create states by repeating state self._num_random_action_selection times
